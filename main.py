...
logging.basicConfig(filename='example.log', level=logging.DEBUG, filemode='w', format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
def train(algo: Algorithm, cfg: DictConfig) -> None:
    ...
    analytics_dir = info["analytics_dir"]
    ...
    for i in range(stop_cond.training_iteration):
        ...
        with open("info.txt", "w") as f:
            json.dump(info, f)
        with open(analytics_dir+"//rewardLog.txt","a") as f:
            f.write("\nNEW ITERATION,"+str(i))
...
def main(cfg: DictConfig) -> None:
    ...
    analytics_dir = "train_analytics//"+datetime.now().strftime("%m.%d.%Y_%H:%M:%S")
    if os.path.isdir(analytics_dir) == False:
        os.mkdir(analytics_dir)
    with open(analytics_dir+"//customLog.txt","w") as f:
        f.write("")
    with open(analytics_dir+"//rewardLog.txt","w") as f:
        f.write("")
    with open("info.txt","w") as f:
        json.dump({"analytics_dir":analytics_dir,"currentIteration":0},f)
